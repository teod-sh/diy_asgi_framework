...
def _create_background_tasks_instance(*args, **kwargs):
    global _bg_tasks
    if _bg_tasks is not None:
        raise RuntimeError("BackgroundTasks already created")
    _bg_tasks = BackgroundTasks(*args, **kwargs)
    return _bg_tasks

def get_background_tasks():
    if _bg_tasks is None:
        raise RuntimeError("BackgroundTasks not initialized. Call create_instance() first.")
    return _bg_tasks

# support for testing purposes
def _reset_instance():
    global _bg_tasks
    _bg_tasks = None